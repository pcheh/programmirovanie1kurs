...
namespace ArraysTasks
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            PrintIntArray(GetEvenIndexElements(numbers));

            var clone = CloneIntArray(numbers);
            Console.WriteLine($"Массив-клон: ");
            PrintIntArray(clone);
            ...
            Console.Write($"Массив с нулями вместо нечётных чисел: ");
            PrintIntArray(clone);
        ...
        static int[] GetEvenIndexElements(int[] array)
        {
            if (array.Length == 0)
                return new int[0];
